...
const props = defineProps({ breadcrumb: Array, permisos: Object, rol: Object, rol_permisos: Array });
const form = useForm({
    'name': props.rol.name,
    'permisos': props.rol_permisos
});

const submit = () => {
    form.put(route('dashboard.roles.update', props.rol.id));
}
...
<template>
    ...
    <TitlePage :breadcrumb="breadcrumb" title="Editar Rol"/>
    ...
    <div class="max-w-[600px] mx-auto bg-light-card dark:bg-dark-card p-8 border border-slate-200 dark:border-dark-border/5">
        <form @submit.prevent="submit" class="flex flex-col space-y-4">
            ...
            <button :disabled="form.processing" type="submit" class="text-white text-[14px] cursor-pointer rounded-md px-6 py-2 bg-dark-action-primary hover:brightness-90 dark:hover:bg-dark-action-primary/60 transition-all duration-200">Actualizar</button>
        </form>
    </div>
</template>
