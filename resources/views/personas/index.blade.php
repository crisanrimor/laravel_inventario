...
@section('content')
    ...
    <x-common.component-card title="Nueva Persona">
        <div class="flex items-center gap-5">
            <a href="{{route('personas.create')}}"><x-ui.button size="sm" variant="primary" :startIcon="$BoxIcon">Crear Persona</x-ui.button></a>
        </div>
    </x-common.component-card>

    <div class="mt-5">
        <x-common.component-card title="Lista de Personas">
                ...
                <table class="w-full mb-5">
                    <thead class="px-6 py-3.5 border-t border-gray-100 border-y bg-gray-50 dark:border-white/[0.05] dark:bg-gray-900">
                        <tr>
                            ...
                            <th class="px-6 py-3 font-medium text-gray-500 sm:px-6 text-theme-xs dark:text-gray-400 text-start">Telefono</th>
                            <th class="px-6 py-3 font-medium text-gray-500 sm:px-6 text-theme-xs dark:text-gray-400 text-start">Dirección</th>
                            <th class="px-6 py-3 font-medium text-gray-500 sm:px-6 text-theme-xs dark:text-gray-400 text-start">Tipo Persona</th>
                        ...
                        @foreach ($personas as $persona)
                            <tr class="border-b border-gray-100 dark:border-white/[0.05]">
                                <td class="px-4 sm:px-6 py-3.5">
                                    <p class="text-gray-700 text-theme-sm dark:text-gray-400">{{$persona->nombre}}</p>
                                </td>
                                <td class="px-4 sm:px-6 py-3.5">
                                    <p class="text-gray-700 text-theme-sm dark:text-gray-400">{{$persona->email}}</p>
                                </td>
                                <td class="px-4 sm:px-6 py-3.5">
                                    <p class="text-gray-700 text-theme-sm dark:text-gray-400">{{$persona->telefono}}</p>
                                </td>
                                <td class="px-4 sm:px-6 py-3.5">
                                    <p class="text-gray-700 text-theme-sm dark:text-gray-400">{{$persona->direccion}}</p>
                                </td>
                                <td class="px-4 sm:px-6 py-3.5">
                                    <p class="text-gray-700 text-theme-sm dark:text-gray-400">{{$persona->tipo_persona->name}}</p>
                                </td>
                                <td class="px-4 sm:px-6 py-3.5">
                                    <div class="flex gap-2.5">
                                        ...
                                        <button @click="openModal({{$persona->id}})">
            ...
            <div class="flex flex-col px-2 overflow-y-auto modal-content custom-scrollbar">
                ...
                <div class="mt-8 modal-body">
                    <p class="text-sm text-gray-500 dark:text-gray-400">
                        ¿Estás seguro que quieres eliminar la persona?
                    </p>
                </div>
...
@push('scripts')
    <script>
        const openModal = (id) => {
            ...
            formModal.action = `{{url('/personas')}}/${id}`
        };
